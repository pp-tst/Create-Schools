...
class service_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		$output .= sprintf(
			...
			'<svg role="img" class="symbol" aria-hidden="true" focusable="false"><use xlink:href="' . get_template_directory_uri() . '/img/icons.svg#' . strtolower( $item->title ) . '"></use></svg><span>' . $item->title . '</span>'
		);
	}
	public function end_el( &$output, $item, $depth = 0, $args = array() ) {
		$output .= "</li>\n";
